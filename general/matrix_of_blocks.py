...
class Matrix(object):
    ...
    # noinspection PyTypeChecker
    def set_block_to_background(self, pos_x: int, pos_y: int) -> None:
    ...
    # noinspection PyTypeChecker
    def set_block_to_wall(self, pos_x: int, pos_y: int) -> None:
        if 0 <= pos_x <= self.columns - 1 and 0 <= pos_y <= self.rows - 1:
            self.two_dim_list[pos_x][pos_y] = Wall(pos_x, pos_y)

    # changes particular block's type in two dimensional list (matrix)
    # noinspection PyTypeChecker
    def change_blocks_type(self, pos_x: int, pos_y: int, new_blocks_type: str) -> None:
        ...
        if new_blocks_type == Blocks.BACKGROUND:
            ...
            self.two_dim_list[pos_x][pos_y] = Wall(pos_x, pos_y)
        elif new_blocks_type == Blocks.PATH:
            # TODO
            pass
